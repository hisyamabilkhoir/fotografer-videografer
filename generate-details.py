...
<body class="loading noise-overlay">
  ...
  <script type="module">
    import { fetchData, getBasePath, resolvePath } from '../../assets/js/utils.js';
    ...
    document.addEventListener('loaderComplete', async () => {
      ...
      // Get projects with the same category (excluding current)
      let related = data.projects.filter(p => p.category === currentCategory && p.slug !== currentSlug);

      // If we have fewer than 3 related projects, fill the remaining spots with other projects
      if (related.length < 3) {
        const addedSlugs = new Set(related.map(p => p.slug));
        addedSlugs.add(currentSlug);

        const extra = data.projects.filter(p => !addedSlugs.has(p.slug)).slice(0, 3 - related.length);
      ...
      if (related.length) {
        container.innerHTML = related.map(p => `
          <a href="${basePath}/portfolio/${p.slug}/" class="featured-card" data-tilt>
            <img src="${resolvePath(p.cover)}" alt="${p.title}" loading="lazy">
      ...
      // Re-initialize GSAP ScrollTrigger for the newly added children
      if (typeof gsap !== 'undefined' && typeof ScrollTrigger !== 'undefined') {
        ...
        // Fallback: make children visible if GSAP is not present
...
def generate_all():
    ...
    for p in projects_data["projects"]:
        ...
        for img in p["images"]:
            ...
            imgs_html.append(
                f'<div class="masonry-item-wrapper" data-animate="fade-up">'
                f'<div class="masonry-item" data-lightbox="project-gallery" data-src="{rel_img}" style="cursor:pointer;">'
                f'<img src="{rel_img}" alt="{p["title"]} Gallery Image" loading="lazy">'
                f'</div>'
                f'</div>'
            )
    ...
    for ac in activities_data["activities"]:
        ...
        for img in ac["images"]:
            ...
            imgs_html.append(
                f'<div class="masonry-item-wrapper" data-animate="fade-up">'
                f'<div class="masonry-item" data-lightbox="activity-gallery" data-src="{rel_img}" style="cursor:pointer;">'
                f'<img src="{rel_img}" alt="{ac["title"]} Gallery Image" loading="lazy">'
                f'</div>'
                f'</div>'
            )
        html = html.replace("{{GALLERY_IMAGES}}", "\n".join(imgs_html))
